...
const Navbar = () => {
  ...
  const handleLogout = async () => {
    try {
      await logout();
      toast.success("Logout successful!");  // ✅ Logout er por toast
    } catch (error) {
        ...
        <div className="hidden md:flex items-center gap-6 text-lg font-medium">
          ...
          {user ? (
            <div className="relative group cursor-pointer">
              ...
              {/* Dropdown */}
              <div
                className="absolute right-0 top-full mt-2 w-44 bg-white dark:bg-gray-800 
                  shadow-lg rounded-md opacity-0 group-hover:opacity-100 
                  group-hover:visible invisible transition-all duration-200 z-50"
              >
                <p className="px-4 py-2 border-b dark:border-gray-700 font-semibold">
                  {user.displayName}
                </p>
                <Link
                  to="/dashboard"
                ...
                <button
                  onClick={handleLogout}  // ✅ Modified here
        ...
        <div className="md:hidden px-4 pb-4 space-y-4 text-lg">
          ...
          {user ? (
            <div className="mt-3">
              ...
              <Link to="/dashboard" className="block mt-2 hover:text-blue-500">
                Dashboard
              </Link>
